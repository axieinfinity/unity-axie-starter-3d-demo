...
public class CharacterInputController : MonoBehaviour
{
	...
	{
		m_IsRunning = false;
        character.animController.PlayAnim(AxieStarter3D.StarterAnimController.AnimName.Idle);
    ...
    {   
	    StartMoving();
		character.animController.PlayAnim(AxieStarter3D.StarterAnimController.AnimName.Run);
	}
	...
	protected void Update ()
    {
        ...
        if(m_Jumping)
        {
			if (trackManager.isMoving)
			{
				...
				{
					m_Jumping = false;
					character.animController.PlayAnim(AxieStarter3D.StarterAnimController.AnimName.RunJump);
				}
				else
				{
			...
			else if(!AudioListener.pause)//use AudioListener.pause as it is an easily accessible singleton & it is set when the app is in pause too
			{
				...
				{
					character.animController.PlayAnim(AxieStarter3D.StarterAnimController.AnimName.IdleJump);
					m_Jumping = false;
				}
			}
        }
